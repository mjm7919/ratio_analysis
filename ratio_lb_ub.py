...
cap.release()
test_params=[]
for param1 in np.arange(1.72,2.04,0.01):
    start_time = time.time()
    print('param1=',str(param1))
#    for param2 in np.arange(1.7,2.04,0.01):
#        print('param2=',str(param2))
    for param2 in np.arange(8,16,1):
        print('param2=',str(param2))
        for kernel_k1 in np.arange(3,9,2):
...
#
                ...
                for k in np.arange(1,n_frames - 1,1):
                    ...
                    lb=np.array(np.abs(np.mean(ratio)-param1*np.std(ratio)))
...
#                
    ...
    #            plot_cv2(full_threshG,'smoothed ratio analysis','png',path)
                ...
                if len(cnts1)==2 and len(cnts2)==2:
                    test_params.append([kernel_k1,kernel_k2,param1,param2])
#                end_time = time.time()
